...
如果由之前的数据版本，可以直接下快速补丁（标记为Only Updated的zip文件）（上个数据版本为1.02.0）
...
- 如果技能有需要特殊目标选择的效果（换人、选卡色、随机效果），主动技能的发动需要勾选特殊发动目标来声明并且提供选项。
...
- 特殊说明：
  ...
  - 选择指令卡：对应的状态应该只为指令卡类型变化和宝具指令卡类型变化。同时该效果涉及到特殊的目标选择，需要在技能中声明特殊目标。
  - 换人：涉及到特殊的目标选择，需要在技能中声明特殊目标。
  - 随机效果：涉及到特殊的目标选择，需要在技能中声明特殊目标。
- 举例说明：
