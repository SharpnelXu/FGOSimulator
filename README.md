...
## BUG修复
- 会在下个Release修复的Bug（v0.51.beta）：
  - Buff相关
    - 不可叠加状态检测现在会忽略掉强制为可叠加的同类Buff（大公三技能）
    - 毅力发动顺序调整为优先消耗可解除的最早施加的毅力，直至全部可解除的毅力消耗完了才消耗不可解除的毅力（也是从最早施加的开始消耗）
    - 伤害反弹现在只在受到过伤害才发动，并且无视回避和无敌，也不再消耗防御者的回避和无敌
  - 关卡编辑
...
- 无效类：由被强化成功率-1000%和弱化耐性1000%分别实现强化无效和弱化无效。
- 魅惑耐性下降：是精神类状态
...
### 注意事项：
- 礼装编号需要为整数。
- 礼装本质为以当前从者为效果发动者的一系列效果，另外在付与状态时不需要勾选
- 礼装付与的状态不用勾选不可解除，会自动标记为不可解除。
...
### BUFF
* 帝王花增殖状态这种复数的可堆叠上限没写
* 防UP和防DOWN上限是分别计算吗？举例500防UP和300防DOWN最后是500-300=200还是400-100=300？
* 无貌之月不会正确解除，这个我懒得写了
